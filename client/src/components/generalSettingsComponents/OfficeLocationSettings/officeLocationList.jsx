...
} from "../../../api/officeLocation"; // Verify this import path matches your project structure
import {
  Search,
  X,
  ArrowUpDown,
...
const DesignationList = ({ selectedOffice, setSelectedOffice }) => {
  ...
  const processedData = useMemo(() => {
    if (!designations) return { data: [], totalPages: 0 };
  ...
  }, [designations, debouncedSearch, sortOrder, page, limit]);
    ...
    <div className="flex flex-col h-full bg-white rounded-xl border border-neutral-200 shadow-sm overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-neutral-100 bg-neutral-50/50 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-neutral-500">Manage Designations</p>
        </div>
        <CustomButton
          label="Add New"
          variant="primary"
          onClick={openAddModal}
          className="!py-1.5 !px-3 !text-xs !h-auto flex items-center gap-1 shadow-sm"
          ...
          <ul className="flex flex-col gap-1">
            {isLoading ? (
              ...
              processedData.data.map((item) => {
                const isActive = selectedOffice?._id === item._id;
                ...
                return (
                  ...
                  >
                    {/* Icon Avatar - Matching Provincial Office Style */}
                    <div
              ...
              <div className="flex flex-col items-center justify-center py-12 text-neutral-400">
                ...
                <p className="text-xs">Try adding a new one</p>
              </div>
            )}
          </ul>
        </div>
